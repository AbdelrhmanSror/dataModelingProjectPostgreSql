...
create_song_plays_table = """create table if not exists songplays  (songplay_id SERIAL PRIMARY KEY,start_time bigint 
NOT NULL ,user_id int , level varchar,song_id varchar ,artist_id varchar ,session_id int ,location varchar,
user_agent varchar) """
...
varchar,year int ,duration numeric) """
...
create_time_table = """create table if not exists time (start_time bigint PRIMARY KEY, hour varchar 
...
%s,%s,%s,%s) on conflict(songplay_id) do nothing"""
